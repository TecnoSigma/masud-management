...
RSpec.describe SubscriptionReactivationJob, type: :job do
  describe '.perform' do
    it 'does not update subscriber status' do
      subscriber = FactoryBot.create(:subscriber)
      subscriber.update_attributes!(status: 'ativado')

      subscription = FactoryBot.create(:subscription, status: 'desativado', subscriber: subscriber)
    ...
    it 'does not update subscriber status when occurs some errors' do
      subscriber = FactoryBot.create(:subscriber)
      subscriber.update_attributes!(status: 'ativado')

      subscription = FactoryBot.create(:subscription, status: 'deativado', subscriber: subscriber)
